import  nextEvo  from "../images/nextevo.png";
import solu from "../images/solu.png";
import mindwell from "../images/mindwell.png";
import chatify from "../images/chatify.png";
import wiin from "../images/wiin.png";
import commerce from "../images/commerce.png";
import music from "../images/music.png";
import todo from "../images/todo.png";
import insta from "../images/insta.png";
import twitter from "../images/twitter.png";
import vfx from "../images/vfx.png";
import spotify from "../images/spotify.png";

export const Bio = {
    name: "Ghofrane Chergui",
    roles: [
     "Web Developer",
    "UI/UX Designer",
    "Graphic Designer"
    ],
    description:
      "A forward-thinking creative with a passion for design, technology, and impactful storytelling. I thrive at the intersection of innovation and aesthetics, where every challenge becomes an opportunity to build something meaningful. Whether crafting user-centered interfaces, shaping digital experiences, or exploring new tools, I bring both curiosity and precision to my work. Thoughtful design, continuous growth, and leaving a unique mark on everything I do that’s what drives me.",
    github: "https://github.com/ghofrane203",
    resume:
      "https://drive.google.com/drive/folders/10JW5hzSTVUBy52J8A4LI_RZHebDRhhaR?usp=sharing",
    linkedin: "https://www.linkedin.com/in/ghofrane-chergui-8a6906262/",
    insta: "https://www.instagram.com/ghofranechergui/",
    facebook: "https://www.facebook.com/ghofrane.ch.482052",
  };
  
  export const skills = [
    {
      title: "Frontend",
      skills: [
        {
          name: "React.js",
          image:
            "https://encrypted-tbn1.gstatic.com/images?q=tbn:ANd9GcQR0XPsnGYjAkavR6ARi4ULiFvxUqNBFksMgbhGCM5bJl7NHzYBDONopbtm88JxkOuHnr86W5h-5eBhszlk3jAoLt2-KCuRWKHU2livJn8",
        },
        {
          name: "JavaScript",
          image:
            "https://upload.wikimedia.org/wikipedia/commons/7/73/Javascript-736400_960_720.png",
        },
        {
          name: "HTML",
          image:
            "https://upload.wikimedia.org/wikipedia/commons/thumb/6/61/HTML5_logo_and_wordmark.svg/1200px-HTML5_logo_and_wordmark.svg.png",
        },
        {
          name: "CSS",
          image:
            "https://upload.wikimedia.org/wikipedia/commons/thumb/d/d5/CSS3_logo_and_wordmark.svg/1452px-CSS3_logo_and_wordmark.svg.png",
        },
        {
          name: "Tailwind CSS",
          image:
            "https://avatars.githubusercontent.com/u/30317862?s=200&v=4",
        },
         {
          name: "Framer Motion",
          image:
            "https://www.ejable.com/wp-content/uploads/2022/04/Framer-Motion-1200x1159.webp",
        },
        {
          name: "Axios",
          image:
            "https://ropercenter.cornell.edu/sites/default/files/styles/800x600/public/Images/Axios_0.PNG?itok=fr1HZ7sF",
        },
      ],
    },
    {
      title: "Backend",
      skills: [
        {
          name: "Node Js",
          image: "https://nodejs.org/static/images/logo.svg",
        },
        {
          name: "Express Js",
          image:
            "data:image/png;base64,",
        },
        {
          name: "Mongoose",
          image: "https://media2.dev.to/dynamic/image/width=1000,height=420,fit=cover,gravity=auto,format=auto/https%3A%2F%2Fdev-to-uploads.s3.amazonaws.com%2Fuploads%2Farticles%2Fl8rbs6hk39ei4m4emawr.png",
        },
        {
          name: "MySQL",
          image:
            "https://raw.githubusercontent.com/devicons/devicon/master/icons/mysql/mysql-original-wordmark.svg",
        },
        
        {
          name: "MongoDB",
          image:
            "https://raw.githubusercontent.com/devicons/devicon/master/icons/mongodb/mongodb-original-wordmark.svg",
        },
      ],
    },
    {
      title: "Graphic Design",
      skills: [
        {
          name: "Photoshop",
          image:
            "https://upload.wikimedia.org/wikipedia/commons/thumb/a/af/Adobe_Photoshop_CC_icon.svg/640px-Adobe_Photoshop_CC_icon.svg.png",
        },
        {
          name: "Illustrator",
          image:
            "https://upload.wikimedia.org/wikipedia/commons/thumb/f/fb/Adobe_Illustrator_CC_icon.svg/1200px-Adobe_Illustrator_CC_icon.svg.png",
        },
        {
          name: "Adobe XD",
          image:
            "https://upload.wikimedia.org/wikipedia/commons/thumb/c/c2/Adobe_XD_CC_icon.svg/800px-Adobe_XD_CC_icon.svg.png",
        },
        {
          name: "InDesign",
          image:
            "https://img.freepik.com/vecteurs-premium/icone-du-logiciel-adobe-indesign_539007-184.jpg",
        },
        {
          name: "Canva",
          image:
            "https://521324.fs1.hubspotusercontent-na1.net/hubfs/521324/__hs-marketplace__/Canva_Circle_Gradient%20(1).jpeg",
        },
      ],
    },
    {
      title: "Others",
      skills: [
        {
          name: "Git",
          image:
            "https://avatars.githubusercontent.com/u/18133?s=200&v=4",
        },
        {
          name: "GitHub",
          image:
            "https://github.githubassets.com/images/modules/logos_page/GitHub-Mark.png",
        },
        {
          name: "VS Code",
          image:
            "https://upload.wikimedia.org/wikipedia/commons/thumb/9/9a/Visual_Studio_Code_1.35_icon.svg/512px-Visual_Studio_Code_1.35_icon.svg.png?20210804221519",
        },
        {
          name: "Postman",
          image:
            "https://yt3.googleusercontent.com/XRzDTgEa9GybH_Uk21E9ri6_iYh-9gbyZzhiEBCnLjISgjTorjMiu7IwpChUMf2lLpEdX6ufDA=s900-c-k-c0x00ffffff-no-rj",
        },
        {
          name: "Figma",
          image:
            "https://upload.wikimedia.org/wikipedia/commons/3/33/Figma-logo.svg",
        },
      ],
    },
  ];
  
  export const experiences = [
    {
      id: 0,
      img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR1ui-1HrmjaTbg9Kj_plZxcqV2iVcLbI0Bbw&s",
      role: "FULL STACK DEVELOPMENT (Internship)",
      company: "ZEN",
      date: "June 2022 – August 2022 ",
      desc: "Contributed to both front-end and back-end development tasks, assisting in building and maintaining web applications using modern technologies.",
      skills: [
        "HTML",
        "CSS",
        "JavaScript",
        "React",
        "Node.js",
        "Express.js",
        "MongoDB",
      ],
    },
    {
      id: 1,
      img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTRbhQkDvyPXLAxU5umsZAuqxa3_vBcQeIkjA&s",
      role: "FULL STACK WEB DEVELOPMENT (Internship)",
      company: "TAV Monastir Airport",
      date: "June 2023 – August 2023 ",
      desc: "Participated in the development of internal web-based tools, supporting the team in both server-side and client-side implementation.",
      skills: [
        "React.js",
        "Node.js",
        "Express.js",
        "MongoDB",
        " UI Enhancements",

      ],
    },
    {
      id: 2,
      img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTLPP3pEydf6om9xmHypjOMaidMBVlyxHdR8A&s",
      role: "FRONTEND DEVELOPER ",
      company: "Freelance",
      date: "January 2023  - Present",
      desc: "Designed and implemented user interfaces for web applications, working with various clients to deliver responsive and accessible UI components.",
      skills: [
        "HTML",
        "CSS",
        "React.js",
        "UX Principles",
        "Angular",
        "Tailwind CSS",
        "Figma",
        
      ],
    },
    {
      id: 3,
      img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRTM8MIkv55jhC_IiMF5VaYMJmE1JzFomUSjA&s",
      role: "MARKETING MANAGER",
      company: "IOT ISIMG CLUB",
      date: "September 2024 – Present",
      desc: "Leading the club’s marketing efforts, including campaign planning, visual branding, and audience engagement across multiple platforms.",
    },
    
  ];
  
  export const education = [
    {
      id: 0,
      img: "https://www.ecoles.com.tn/sites/default/files/universite/logo/logo_isimG.jpg",
      school: "Higher institute of computer science and multimedia of gabes",
      date: "Sep 2021 - juin 2024",
      desc: "I have completed my Bachelor's degree in Computer Science at the Higher Institute of Computer Science and Multimedia of Gabes. Throughout the 6 semesters, I studied a wide range of courses including Data Structures, Algorithms, Object-Oriented Programming, Operating Systems, Database Management Systems, Software Engineering, Statistics, Human-Computer Interaction, Graphic Design, Multimedia, and UI/UX Design. In addition to academics, I was actively involved in student clubs such as Tunisian Programming Lovers, VFX ISIMG, and the Microsoft Tunisian Student Club. I also served as the Marketing Lead of the IoT Club, where I helped organize events and manage promotional content. These experiences allowed me to work on collaborative projects, explore creative technologies, and grow within a team of passionate developers.",
      degree: "Bachelor of Science in Computer Science",
    },
    {
      id: 1,
      img: "https://www.ecoles.com.tn/sites/default/files/universite/logo/logo_isimG.jpg",
      school: "Higher institute of computer science and multimedia of gabes",
      date: "Oct 2024 - Present",
      desc: "I am currently pursuing a Research Master's degree in Computer Science and Multimedia at the Higher Institute of Computer Science and Multimedia of Gabes. My studies focus on advanced topics in software engineering, multimedia systems, AI, and research methodologies. I am actively involved in academic research and collaborative projects that explore the intersection of computing and digital media.",
      degree: "Research Master's in Computer Science and Multimedia",
    },
   
  ];
  
 export const projects = [
  {
    id: 0,
    title: "Cinéma Ticket Reservation App",
    date: "Mar 2024 - Apr 2024",
    description:
      "A full-stack web application for reserving cinema tickets. Built with the MERN stack, it includes user authentication, seat selection, movie schedules, and payment simulation. The UI was carefully designed with Figma and implemented using Tailwind CSS for a seamless experience.",

    tags: ["React", "Node.js", "Express.js", "MongoDB", "Tailwind", "MERN"],
    category: "Web",
    github: "", // add if available
    webapp: "", // add if available
  },
  {
    id: 1,
    title: "Airport Badge Management System",
    date: "Feb 2024 - Mar 2024",
    description:
      "An internal badge management system for airport employees, built with MERN stack. Allows HR to assign and track access badges, verify credentials, and monitor badge history. Designed for secure, real-time access control.",

    tags: ["MongoDB", "Express.js", "React", "Node.js", "Admin Panel"],
    category: "Web",
    github: "",
    webapp: "",
  },
  {
    id: 2,
    title: "Post-Traumatic Stress Management App",
    date: "Jan 2024 - Mar 2024",
    description:
      "An app focused on PTSD management. Includes user registration as psychologist or patient, online consultation scheduling, social interaction between patients, integrated games for relaxation, and a PTSD detection test. Developed with the MERN stack and integrates multimedia support.",
 
    tags: ["React", "MongoDB", "Node.js", "Express.js", "PTSD", "Mental Health"],
    category: "Web",
    github: "",
    webapp: "",
  },
  {
    id: 3,
    title: "E-Learning Platform",
    date: "Nov 2023 - Dec 2023",
    description:
      "A full-featured online learning platform developed using the MERN stack. Features include course uploads, lesson organization, user dashboards, file sharing, and user role management (teacher/student).",

    tags: ["React", "MongoDB", "Node.js", "Express.js", "E-Learning"],
    category: "Web",
    github: "",
    webapp: "",
  },
  {
    id: 4,
    title: "Note App",
    date: "Oct 2023",
    description:
      "A simple and clean note-taking web application with color tagging, categories, and real-time updates. Built using the MERN stack.",

    tags: ["React", "Node.js", "Express.js", "MongoDB"],
    category: "Web",
    github: "",
    webapp: "",
  },
  {
    id: 5,
    title: "Online Shopping Platform",
    date: "Sep 2023",
    description:
      "An e-commerce web application for product listing, cart management, and simulated checkout. Built with the MERN stack and responsive UI design.",
  
    tags: ["React", "Node.js", "MongoDB", "Express.js", "E-Commerce"],
    category: "Web",
    github: "",
    webapp: "",
  },
  {
    id: 6,
    title: "Graphic Design Projects",
    date: "2021 - Present",
    description:
      "Designed professional posters and digital media assets for clients, university events, and the IoT Club using Photoshop, Illustrator, and Canva. Focus on branding, event identity, and promotional visuals.",
 
    tags: ["Photoshop", "Illustrator", "Canva", "Branding", "Poster Design"],
    category: "Design",
    github: "",
    webapp: "",
  },
  {
    id: 7,
    title: "Freelance Platform - UI/UX and Development",
    date: "Ongoing",
    description:
      "Currently working on a freelance platform from scratch, including UI/UX design in Figma and full development using MERN stack. The platform will include user registration, profile management, project posting, and messaging.",
    tags: ["Figma", "UI/UX", "React", "MongoDB", "Node.js"],
    category: "Fullstack",
    github: "",
    webapp: "",
  },
];

  
  export const TimeLineData = [
    { year: 2017, text: "Started my journey" },
    { year: 2018, text: "Worked as a freelance developer" },
    { year: 2019, text: "Founded JavaScript Mastery" },
    { year: 2020, text: "Shared my projects with the world" },
    { year: 2021, text: "Started my own platform" },
  ];